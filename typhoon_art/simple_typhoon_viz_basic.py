...
def load_typhoon_data(csv_path):
    ...
    # 转换SEASON为数字类型
    storms['SEASON'] = pd.to_numeric(storms['SEASON'], errors='coerce')
...
def plot_typhoon_tracks_basic(storms, num_tracks=5):
    """绘制台风轨迹 - 基础版本"""
    # 获取一些示例台风
    ...
    for i, storm_id in enumerate(selected_storms):
        ...
        # 按风速着色散点
        scatter = plt.scatter(storm_data['LON'], storm_data['LAT'],
...
def plot_single_typhoon_basic(storms, storm_index=0):
    """绘制单个台风的详细轨迹 - 基础版本"""
    unique_storms = storms['SID'].unique()
    ...
    # 标记起点和终点
    plt.scatter(storm_data['LON'].iloc[0], storm_data['LAT'].iloc[0], 
              color='green', s=100, marker='o', label='Start')
    plt.scatter(storm_data['LON'].iloc[-1], storm_data['LAT'].iloc[-1], 
...
def main():
    ...
    try:
        ...
        if choice == '2':
            ...
            plt = plot_single_typhoon_basic(storms, storm_index)
        else:
            # 显示多个台风
            num_tracks = int(input("请输入要显示的台风数量 (默认5): ") or "5")
            plt = plot_typhoon_tracks_basic(storms, num_tracks)
